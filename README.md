...
**Note:** For MacOS Catalina users, there is an issue with the libv8/therubyracer gem that requires setting compilation flags separately. If you receive the following error while running the second shell script:
...
Then you will need to run the following commands to fix the libv8 version before re-running `bundle install`
...
Spinning up the docker containers is fast, but not instantaneous Occasionally, running `foreman start` _immediately_ after `docker-compose up -d`, can cause the rails server to fail to start because not all of the containers are ready. For example, the following _may_ not work:
...
_DANGER_: If Caseflow Monitor is incorrectly reporting a dependency issue, you can disable the "Degraded Service" banner with the following code:
